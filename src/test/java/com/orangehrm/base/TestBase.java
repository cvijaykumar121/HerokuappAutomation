...
public class TestBase {
    ...
    public void logPass(String statement, boolean takeScreenshot) {
        if(takeScreenshot) {
            extentTest.log(Status.PASS, statement, MediaEntityBuilder.createScreenCaptureFromBase64String(captureScreenshot()).build());
            logger.info(statement);
        } else {
            extentTest.info(statement);
            logger.info(statement);
    ...
    public void clickElement(By locator, String message) {
        try {
